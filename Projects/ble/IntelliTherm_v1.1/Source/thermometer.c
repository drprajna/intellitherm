...
//JFang //#include "ioCC2541.h"
//JFang #include "hal_i2c.h"
//JFang #include "hal_temp112.h"
#include "ioCC2540.h"
#include "ADS8326.h"
#include <stdarg.h>
#include "osal_snv.h"
...
#define DEFAULT_DESIRED_MIN_CONN_INTERVAL     80 //200 JFang, 80=100ms
...
#define DEFAULT_DESIRED_CONN_TIMEOUT          1000 //JFang, 10s
...
static uint8 attDeviceName[GAP_DEVICE_NAME_LEN] = "Thermo Sensor";
...
//static bool temperatureIntervalConfig = false; //JFang?
 ...
 * JFANG Define
 */
#define TH_NVID_USER_DATA_START 0x80 //£¡¡¶ Start of the USER Configuration NV IDs
#define TH_NVID_USER_DATA_END 0x100 //£¡¡¶ End of the USER Configuration NV IDs

//JFang, don't do it with OSAL
//#if 0
//#define LED1 P1_0       //¶¨ÒåP1.0¿ÚÎªLED1¿ØÖÆ¶Ë
//#define LED2 P1_1       //¶¨ÒåP1.1¿ÚÎªLED2¿ØÖÆ¶Ë
//#define LED3 P1_4       //¶¨ÒåP1.4¿ÚÎªLED3¿ØÖÆ¶Ë
//#define LED4 P0_1       //¶¨ÒåP0.1¿ÚÎªLED4¿ØÖÆ¶Ë
///****************************************************************************
//* Ãû    ³Æ: InitLed()
//* ¹¦    ÄÜ: ÉèÖÃLEDµÆÏàÓ¦µÄIO¿Ú
//* Èë¿Ú²ÎÊý: ÎÞ
//* ³ö¿Ú²ÎÊý: ÎÞ
//****************************************************************************/
//void InitLed(void)
//{
//    P1DIR |= 0x13;      //P1.0¡¢P1.1¡¢P1.4¶¨ÒåÎªÊä³ö
//    
//    //P0SEL |= ~0x01;
//    //P0DIR |= ~0x01;     //P0.1¶¨ÒåÎªÊäÈë
//    //P0INP |= ~0x01;     //´ò¿ªP0.1ÉÏÀ­µç×è
//    //P0DIR |= 0x02;      //P0.1¶¨ÒåÎªÊä³ö    
//}
//#endif

/****************************************************************************
* Ãû    ³Æ: DelayMS()
* ¹¦    ÄÜ: ÒÔºÁÃëÎªµ¥Î»ÑÓÊ±£¬ÏµÍ³Ê±ÖÓ²»ÅäÖÃÊ±Ä¬ÈÏÎª16M(ÓÃÊ¾²¨Æ÷²âÁ¿Ïàµ±¾«È·)
* Èë¿Ú²ÎÊý: msec ÑÓÊ±²ÎÊý£¬ÖµÔ½´ó£¬ÑÓÊ±Ô½¾Ã
* ³ö¿Ú²ÎÊý: ÎÞ
****************************************************************************/
...
void Thermometer_Init( uint8 task_id )
{
  ...
  */
  ...
  // ´®¿Ú³õÊ¼»¯ ²¨ÌØÂÊÄ¬ÈÏÊÇ115200, ÐÎ²ÎÊÇ»Øµ÷º¯Êý  
  NPI_InitTransport(NpiSerialCallback);//JFang, enable echo
  NPI_WriteTransport("IntelliTherm_Init\r\n", 19);  
  ...
  {
    //JFang, orig: Device doesn't start advertising until button is pressed
...
//JFang
  ...
  /*
  ÉèÖÃÒ»¸öÈÎÎñ£¬ ÕâÃ´×öµÄÄ¿µÄÊÇ°´ÕÕ¶àÈÎÎñ´¦ÀíµÄ·½·¨À´×ö
  Thermometer_ProcessEvent ¾ÍÊÇ´¦Àí TH_START_DEVICE_EVT
  */
}
...
//JFang, Õâ¸öÊÇÎÒÃÇµÄÓ¦ÓÃ³ÌÐòµÄÊÂ¼þ´¦Àíº¯Êý
uint16 Thermometer_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  //JFang, SYS_EVENT_MSG ÕâÊÇÏµÍ³ÊÂ¼þ±ÈÈç°´¼üÊÂ¼þÀ¶ÑÀ¶ÁÐ´ÊÂ¼þ´¦Àí£¬¶¼»áÖÃÕâ¸öÊÂ¼þ
  ...
  // Õâ¸öÊÇÎÒÃÇÓ¦ÓÃ³ÌÐò×Ô¶¨ÒåµÄÊÂ¼þ£¬TH_START_DEVICE_EVT µÄÖµ±»¶¨ÒåÎª 0x0001£¬ 
  // Êµ¼ÊÉÏÎÒÃÇ¿ÉÒÔ¶¨Òå 16¸öÊÂ¼þ£¬ µÚÒ»µÄÊ±ºòÊÇÒÔÎ»À´¶¨ÒåµÄ
  // Õâ¸ö TH_START_DEVICE_EVT ¾ÍÊÇÔÚThermometer_Init³õÊ¼»¯º¯Êý×îºóÒ»ÐÐÉèÖÃµÄÊÂ¼þ
  ...
  HalLedSet( HAL_LED_3 , HAL_LED_MODE_ON );
    ...
    if(NV_OPER_FAILED == ret8)
    {
        ...
        SerialPrintf("NV_OPER_FAILED\r\n");
    } 

    // ÏÔÊ¾°´¼ü´ÎÊý    
    SerialPrintf("count=%d & %d\r\n",ret8, sys_config.count);      
    
     
    // ·µ»ØÕâ¸ö£¬ ¸æËßosalÕâ¸öÊÂ¼þÄãÒÑ¾­´¦ÀíÁË
 ...
 /*   TR0 = TR0_ADCTM;
   ...
   */
   //JFang thermometerCelcius = HalTemp112ReadTemp(); 
 ...
 *                 HAL_KEY_SW_2
 *                 HAL_KEY_SW_1
 *°´¼üÒ»´Î»á½øÀ´Á½´Î£¬µÚÒ»´Î´«Èë¼üÖµ£¬µÚ¶þ´Î´«Èë0ÖµÓÃÀ´È¡Ïû³¤°´ÊÂ¼þ¡£
 *¾ßÌåÔ­ÀíÊÇ£¬°´¼üÊÇÖÐ¶ÏµÄ£¬ÖÐ¶ÏÒ»´ÎºóÁ¢¼´¸ÄÎª100ms²éÑ¯£¬Èç¹û100ms²éÑ¯µ½Ã»ÓÐ
 *±ä»¯£¬ÔòÊ²Ã´Ò²²»·¢Éú£¬¼ÌÐø100ms²éÑ¯¡£Èç¹û²éÑ¯µ½±ä»¯¾ÍÔÙ·¢³öÒ»´ÎÊÂ¼þ¡£
 * @return  none
 */
uint8 test = 0;
...
static void thermometer_HandleKeys( uint8 shift, uint8 keys )
{
  ...
  //JFang, NPI_WriteTransport( keys, 1);

  // smartRF¿ª·¢°åÉÏµÄS1 ¶ÔÓ¦ÎÒÃÇÔ´ÂëÉÏµÄHAL_KEY_SW_6
  if ( keys & HAL_KEY_SW_6 )
  {
    //SerialPrintf("HAL_KEY_SW_6\r\n"); 
    //test++;    
    //°´¼ü´ÎÊýÀÛ¼Ó
    sys_config.count++;
    ...
    // °ÑÊý¾Ý½á¹¹±£´æµ½flash
    uint8 ret8=0;
    ret8=osal_snv_write(0x80, sizeof(SYS_CONFIG), &sys_config); 
    ...
    // ÏÔÊ¾°´¼ü´ÎÊý
    //HalLcdWriteStringValue("KeyCount = ", sys_config.count, 10, HAL_LCD_LINE_5 );
    SerialPrintf("count=%d & %d\r\n",ret8, sys_config.count);

    // Õâ¸öÊ±ºò£¬Èç¹ûÄã°´ÏÂ¿ª·¢°åµÄ¸´Î»°´¼ü»òÕßÖØÐÂÉÏµç£¬½«»áÏÔÊ¾×îÐÂµÄ°´¼ü´ÎÊý
    
    //JFang, HalTemp112ReadTemp();
...
static void thermometerImeasNotify(void)
{
 ...
 // NPI_WriteTransport(buf,6);
  ...
  //JFang, temperature = HalTemp112ReadTemp();
...
#else// Í¨¹ýCHAR6 ÌØÕ÷Öµ·¢ËÍ³öÈ¥£¬ ÕâÀïÓÐÁ½ÖÖ·½·¨
#if 0 // ÕâÖÖËÙ¶ÈÂý SimpleProfile_SetParameter           
        //simpleBLEChar6DoWrite2 = FALSE;
        SimpleProfile_SetParameter( SIMPLEPROFILE_CHAR6,numBytes, buf );
#else // ÕâÖÖËÙ¶È¿ì GATT_Notification
        static attHandleValueNoti_t pReport;
        pReport.len = sendBytes;

        /*
// Ò²ÐíÄú»áÄÉÃÆÕâ¸öÖµ 0x0035 ÔõÃ´À´µÄ£¬Êµ¼ÊÉÏ£¬AmoÎÒÒ²±È½ÏÓôÃÆ£¬ ÄÇÊµ¼ÊÉÏ£¬ 
ÎÒÊÇÍ¨¹ý Ö÷»úÁ¬½ÓÕâ¸ö´Ó»úºó»ñµÃµÄ£¬ È»ºóÔÙÐ´µ½ÕâÀïÀ´£¬ ×÷ÎªÍ¨ÖªµÄhandleÖµ£¬
-------------AmoMcu   2014.08.17  ---------------------------------------------
        */
...
// ´®¿Ú»Øµ÷º¯Êý£¬ ÏÂÃæ°Ñ¸Ã»Øµ÷º¯ÊýÀïÊµÏÖµÄ¹¦ÄÜ½²½âÒ»ÏÂ
/*
1, Ë¼Â·:  µ±´®¿ÚÊÕµ½Êý¾Ýºó£¬¾Í»áÂíÉÏµ÷ÓÃÒÔÏÂ»Øµ÷º¯Êý£¬ÔÚÊµ¼Ê²âÊÔÖÐ·¢ÏÖ£¬´Ë»Øµ÷
º¯Êýµ÷ÓÃÆµ·±£¬ Èç¹ûÄã²»Ö´ÐÐNPI_ReadTransportº¯Êý½øÐÐ¶ÁÈ¡£¬ ÄÇÃ´Õâ¸ö»Øµ÷º¯Êý¾Í»á
Æµ·±µØ±»Ö´ÐÐ£¬µ«ÊÇ£¬ÄãÍ¨¹ý´®¿Ú·¢ËÍÒ»¶ÎÊý¾Ý£¬ Äã±¾ÒâÊÇÏë´¦ÀíÕâÒ»ÍêÕûÒ»¶ÎµÄÊý¾Ý£¬ËùÒÔ£¬
ÎÒÃÇÔÚÏÂÃæÒýÈëÁËÊ±¼äµÄ´¦Àí·½·¨£¬ Ò²¼´½ÓÊÕµÄÊý¾Ý¹»¶à»òÕß³¬Ê±£¬¾Í¶ÁÈ¡Ò»´ÎÊý¾Ý£¬ 
È»ºó¸ù¾Ýµ±Ç°µÄ×´Ì¬¾ö¶¨Ö´ÐÐ£¬Èç¹ûÃ»ÓÐÁ¬½ÓÉÏ£¬¾Í°ÑËùÓÐÊý¾Ýµ±×öATÃüÁî´¦Àí£¬ Èç¹ûÁ¬½Ó
ÉÏÁË£¬¾Í°ÑÊý¾ÝËÍµ½¶Ô¶Ë¡£  ---------------amomcu   2014.08.17
*/
static void NpiSerialCallback( uint8 port, uint8 events )
{
    ...
    if (events & (HAL_UART_RX_TIMEOUT | HAL_UART_RX_FULL))   //´®¿ÚÓÐÊý¾Ý
    {
        ...
        if(numBytes > 0)
        {
            //JFang, uint8 *buffer = osal_mem_alloc(numBytes);            
          //ÉêÇë»º³åÇøbuffer
            uint8 *buffer = osal_mem_alloc(numBytes+5);
            if(buffer)
            {
              *buffer='E';
              *(buffer+1)='C';
              ...
              *(buffer+4)=':';
              //¶ÁÈ¡¶ÁÈ¡´®¿Ú»º³åÇøÊý¾Ý£¬ÊÍ·Å´®¿ÚÊý¾Ý   
                NPI_ReadTransport(buffer+5,numBytes);  
                
                //°ÑÊÕµ½µÄÊý¾Ý·¢ËÍµ½´®¿Ú-ÊµÏÖ»Ø»· 
                NPI_WriteTransport(buffer,numBytes+5);  //JFang
                
//                int8 ret8 = osal_snv_write(TH_NVID_USER_DATA_START, numBytes+5, buffer);
//                if(SUCCESS==ret8)
//                {
//                  NPI_PrintString("Save to snv ID success");
//                }
//                else
//                {
//                  SerialPrintf("ret8=%d\r\n",ret8); 
//                }
                ...
                osal_mem_free(buffer);//ÊÍ·ÅÉêÇëµÄ»º³åÇø                
            }
        }
    }
}
